...
def plotCD(fig, data, reg1, reg2, log):
	"""
    Метод, ответственный за построение графиков поверхностей регрессии.
	Рекомендуется установить делитель интервала в соответствии с исходными данными. 
	"""
	...
	for i in range (2):
		...
		ax.set_title("Производительность в постоянном масштабе" if i==0 else "Доходность по переменному масштабу", fontweight="bold")
		ax.set_xlabel('K', fontweight="bold")
		ax.set_ylabel('L', fontweight="bold")
		ax.set_zlabel('Y', fontweight="bold")
		handles, labels = ax.get_legend_handles_labels()
		ax.legend(handles, labels)
		red_patch = mpatches.Patch(color='red', label='Исходные точки данных')
		plot_patch = mpatches.Patch(color="orange" if i==0 else "blue", label="Поверхность регрессии")
...
def getData(file, log, d=';'):
	"""
	Получить файл данных csv на основе имени
	"""
	data = {"Y": [],
			"K": [],
			"L": [],
...
class RegressionModel:
	...
	def cov(self, a, b):
		"""
		Метод расчета для расчета ковариации
		"""
		cov = 0.0
	...
	def getRes(self, y, x1, b0, b1, x2=None, b2=None, x3=None, b3=None):
		"""
		Получить остатки вычисленной регрессии.
		"""
		res = []
		yp = []
		if (x2 is None):
	...
	def CD(self):
		...
		self.model = model
		res = {"Коэффициенты регрессии": model,
			   "Стандартные ошибки": SE,
			   "t-статистика": t_stat,
			   "Коэффициент детерминации": R2,
			   "Коэффициент детерминации (скорректированный)": R2_adj,
			   "Доказательство F": F,
			   "Статистика Дурбина-Уотсона": dw_test,
			   "Тест Жарке-Бера": jb_test,
			   "Номер условия X^tX": self.cond}

		names_stat = ["Коэффициенты регрессии", "Стандартные ошибки", "t-статистика", "Коэффициент детерминации", "Коэффициент детерминации (скорректированный)"
			, "Доказательство F", "Статистика Дурбина-Уотсона", "Тест Жарке-Бера","Номер условия X^tX"]
		print("{0}\n{1:^103}\n{2}".format("=" * 103, "Краткое изложение регрессии", "=" * 103))
...
def model():
	...
	while(True):
		try:
			file = input("Укажите имя файла данных: ")
			ans = input("Применять натуральный логарифм? (0-ДА, 1-НЕТ): ")
			while (ans not in ("1", "0")):
				print("Input 0 for YES and 1 for NO!\n")
				ans = input("¿Применить натуральный логарифм? (0-ДА, 1-НЕТ): ")
			log = ans == "1"
